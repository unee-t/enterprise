...
function checkTableName($shortTName, $type=false)
{
	...
	if ("Manage_Unee_T_Users" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("admin_rights" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("uneet_enterprise_users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("admin_members" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("admin_users" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("Manage_Users___Unee_T_Enterprise_UI" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("uneet_enterprise_organizations" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("ut_user_types" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("person_statuses" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("person_salutations" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("property_groups_countries" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Manage_Unit_Names" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("ut_unit_types" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("ut_property_types" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Manage_User_Types" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("ut_user_role_types" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Manage_User_Default_Visibility" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("Super Admin - Manage API Keys");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
function SetAuthSessionData($pUsername, &$data, $password, &$pageObject = null, $fireEvents = true )
{
	...
	$_SESSION["_Manage Unee-T Users_OwnerID"] = $data["organization_id"];
		$_SESSION["_Manage Users - Unee-T Enterprise UI_OwnerID"] = $data["organization_id"];
		$_SESSION["_Manage Unit Names_OwnerID"] = $data["organization_id"];
